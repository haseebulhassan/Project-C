//Font Chip Part Number: ER3300-1
...
{uchar code text[]={"深圳旭日东方科技有限公司"};
	uchar code 	text1[]={" ER-TFT035-6  320RGB*480 DOTS"};
		uchar code 	text2[]={"EASTRISING WWW.BUYDISPLSY.COM"};
	uchar i=0; uint x=54,y=20;
...
uint i= 0; uint x=0,y=0;    uchar k,k1;  
	...
	while((text[i])>0x00)
	{
		if(text[i]>=0xa1&&text[i+1]>=0xa1&&text[i]<=0xf9)
		{
			...
			if(x>=319)
			{y+=16;x=0;
			if(y>=479){y=0;x=0;ClearScreen(0xffff);}
			}
		}
...
/*	//dilplay all GB12345 font  	
		...
		{	fontaddr = GB2312_Addr(k,k1); 
			...
			if(x>=319)
			{y+=16;x=0;
			if(y>=479){y=0;x=0;LCD_clear(0xffff);}
			}
		 }
*/
